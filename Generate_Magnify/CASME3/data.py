...
def get_paths_ABC(config, mode,frameA,frameC):
    ...
    if config.cursor_end > 0 or 'test' in mode:
        ...
        dir_C = os.path.join(dir_root, frameC)
        #files_A = sorted(os.listdir(dir_A), key=lambda x: str(x.split('_')[0]))#int(x.split('.')[0]))
...
class DataGen():
    ...
    def gen_test(self, anchor=None):
        ...
        for _ in range(self.batch_size):
            unit_A = load_unit(self.paths[0])
            unit_C = load_unit(self.paths[1])
...
def get_gen_ABC(config, im1, im2, mode='train'):

    paths_A = get_paths_ABC(config, mode,im1,im2)
...
def numpy2cuda(array):
    tensor = torch.from_numpy(np.asarray(array)).float().cuda()#CPU上去掉.cuda()
    return tensor
